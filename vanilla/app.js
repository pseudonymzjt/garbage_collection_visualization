...
const LEVELS = [
  ...
  // ---- Level 3: 堆内存强引用 ----
  {
    ...
    javaCode: [
      ...
      { text: '// 赋值给了长生命周期的全局变量，导致 Root → bigBuffer 冗余链路', activeEdge: 'e-root-buf', commentText: '// 冗余链路已断开 — Root 不再直接引用 bigBuffer' },
      { text: '', alwaysNormal: true },
      { text: '    public void clear() {', alwaysNormal: true },
      { text: '        // 切断核心引用 (HeavyComp → bigBuffer)', alwaysNormal: true },
      { text: '        // this.bigBuffer = null; // 需要切断此引用', activeEdge: 'e-comp-buf', commentText: '        this.bigBuffer = null; // 已释放 — HeavyComp 不再引用 bigBuffer' },
      { text: '    }', alwaysNormal: true },
      { text: '}', alwaysNormal: true },
    ],
